...
const InstaPDF = (props) =>{
    ...
    const handleKeyPrint = (e)=>{
      if(e.ctrlKey && e.key === ','){
        console.log("Print iniitaed...")
        document.getElementById("textprintbutton").click()
      }
    }
    const onSubmit = (data) => {
      console.log(data);
    ...
    <>
      ...
      <div className='absolute flex flex-col w-[20vw] overflow-hidden h-[4.5vh] top-[-12px] right-17 hover:h-auto'>
        ...
        <div className='hover:h-auto hover:w-auto'>
          ...
          <div className='hover:h-8 w-auto bg-white p-2 cursor-pointer' onClick={()=> setColor('bg-white text-white')}>White</div>
          <div className='hover:h-8 w-auto bg-slate-800 p-2' onClick={()=> setColor('bg-slate-800 text-white')}>Dark</div>
          <div className='hover:h-8 w-auto bg-blue-500 p-2' onClick={()=> setColor('bg-blue-700 text-white')}>Blue</div>
          <div className='h-8 w-auto bg-green-500 p-2' onClick={()=> setColor('bg-green-700')}>Green</div>
          <div className='h-8 w-auto bg-red-500 p-2' onClick={()=> setColor('bg-red-700 text-white')}>Red</div>
          <div className='h-8 w-auto bg-orange-500 p-2' onClick={()=> setColor('bg-orange-700 text-white')}>Orange</div>
        </div>
        {/* <div></div>
        <div></div>
        <div></div> */}
      </div>
        <div ref={ComponentRef} className={`w-[50vw] max-sm:w-full print:w-[20cm] print:h-[28.7cm] ${color} print:m-auto print:my-3 print:text-[1.4rem] my-2 p-6 border-4 border-cyan-500  flex justify-start items-center gap-3 flex-col textInputInstaPDF`}>
        ...
        <div className={` max-sm:w-full print:pl-6 flex justify-start items-center gap-3 flex-col text-white text-center`}>
          <h2>Shortcut for printing the below text while cursor on the text is : <b>Ctrl + ,</b></h2>
          <textarea ref={Component2Ref} value={textareaValue} onChange={handleTextareaChange} onKeyDown={handleKeyPrint} name="" id="" className={`w-[50vw] max-sm:w-full min-h-[40vh] print:w-[20cm] print:min-h-[25.7cm] bg-[#ffffff41] print:mx-2 print:my-7 print:text-[28px] my-4 p-6 border-4 border-cyan-500 `}></textarea>
          <button type="button" id='textprintbutton' onClick={useReactToPrint({content: ()=> Component2Ref.current})} className='border-1 bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded text-white'>Print / Save as PDF</button>
        </div>
      </div>
    </>
  );
