...
export const FunctionInput: React.FC<FunctionInputProps> = ({ 
  ...
  const handleExecute = async () => {
    try {
      ...
      const executionResult = onExecute(formData);
      ...
      {localResult !== null && localResult !== undefined && (
        ...
        >
          <h4 className={`text-sm font-medium mb-2 ${
            uiLevel !== 'skeleton' ? uiLevel === 'futuristic' ? 'text-green-400' : 'text-gray-700 dark:text-gray-300' : ""
          }`}>
            Output:
          </h4>
          <pre className={`text-sm font-mono ${
            uiLevel !== 'skeleton' ? uiLevel === 'futuristic' ? 'text-green-300' : 'text-gray-900 dark:text-gray-100' : ""
          }`}>
            {typeof localResult === 'object' ? JSON.stringify(localResult, null, 2) : String(localResult)}
          </pre>
        </motion.div>
      )}
    </motion.div>
