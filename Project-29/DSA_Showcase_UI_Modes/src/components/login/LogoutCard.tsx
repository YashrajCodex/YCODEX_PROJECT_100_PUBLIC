...
import { Algorithims, useBookmark } from "@/contexts/BookmarkContext";
  ...
  useEffect(() => {
    if (bookmarks) {
      setBookmarkedProblems(
        bookmarks.filter((algo) => algo.userEmail === user.email)
      );
    }
  }, [bookmarks, user.email]);
  // console.log(bookmarkedProblems);
  const getCardStyles = () => {
    switch (uiLevel) {
      case "skeleton":
        return "";
    ...
    <>
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className={`mt-2 ${getTextStyles()}`}>
      ...
      <motion.div
        initial={uiLevel !== "skeleton" && { opacity: 0, scale: 0.9 }}
        animate={uiLevel !== "skeleton" && { opacity: 1, scale: 1 }}
        className={`max-w-7xl mx-auto my-3 rounded-lg p-8 ${getCardStyles()}`}
      >
